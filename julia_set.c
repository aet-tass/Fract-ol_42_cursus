...
/*   Updated: 2023/03/08 00:23:38 by aet-tass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_color_julia(t_mlx *mlx)
{
	...
	color = mlx->iter % 4 * 0xFF0000 + mlx->iter % 8 * 0x00FF00
		+ mlx->iter % 16 * 0x0000FF;
...
void	julia_set(t_mlx	*mlx)
{
	...
	mlx->addr_ptr = mlx_get_data_addr(mlx->img_ptr,
			&mlx->bit_per_pixel, &mlx->line_lenght, &mlx->endian);
	mlx->max_iter = 50;
	mlx->win.i = 0;
